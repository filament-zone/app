<script lang="ts">
	...
	let { subPages }: ISubPageMenuBarProps = $props();
	...
	<div class="flex flex-row gap-4">
		{#each Object.entries(subPages) as [key, subRoute]}
			{#if typeof subRoute === 'string'}
				{#if key !== 'ROOT'}
					<div
						class:selected={subRouteSelected(subRoute)}
						class="py-2 flex items-center"
						onclick={handleSubRouteSelect.bind(null, subRoute)}
						aria-hidden="true"
					>
						<Typography
							dataTestId={subRoute}
							variant="h6"
							classNames="cursor-pointer selected capitalize"
			...
			{:else}
				<div
					class:selected={subRouteSelected(subRoute.ROOT)}
					class="py-1 flex items-center"
					onclick={handleSubRouteSelect.bind(null, subRoute.ROOT)}
					aria-hidden="true"
				>
					<Typography
