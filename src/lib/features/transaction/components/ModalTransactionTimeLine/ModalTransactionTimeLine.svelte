<script lang="ts">
	...
	let tx = $derived($transactions.find((tx) => tx.txHash === state?.txHash));
...
<Modal classNames="max-w-96" closeOnClickOutside={false}>
	...
	{#snippet content()}
		<div class="flex flex-col justify-between h-full">
			{#if state?.config?.common?.title}
				<Typography variant="h6" classNames="mb-4">{state?.config.common.description}</Typography>
				<Divider classNames="mb-8" />
				<Typography variant="h6" classNames="mb-4">Transaction Status</Typography>
			{/if}
			{#if tx?.error}
				<TxTimeLineItem
	...
	{/snippet}
	{#snippet footer()}
		<Button class="ml-auto mt-8" onclick={closeModal}>Close</Button>
	{/snippet}
</Modal>
